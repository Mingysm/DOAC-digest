...
# 3. LLM
# ---------------------------------------------------------------------------
MAP_PROMPT = """你是《The Diary Of A CEO》的深度访谈研究员。
下面是一期节目的文字稿【第 {i}/{n} 部分】。

你的任务不是写泛泛的摘要，而是建立一个“高信息密度事实库”，供下一阶段生成完整中文节目笔记。
这一阶段尤其要避免因为过度压缩而丢失细节。
...
- 历史事件或个人经历
尽量保留原始数字和上下文。
如果只是嘉宾声称、而文字稿没有给出证据，也标记为“嘉宾声称”。

### 4. 机制与因果关系
如果嘉宾解释了“为什么 A 会导致 B”，把逻辑链写出来。
例如：
A → B → C
并说明这是嘉宾提出的解释还是文字稿中的明确事实。
...
### 9. 本段最值得保留的 3 个信息
最后用 3 条短句指出：如果读者以后只复习这一段，最应该记住什么。
...
- 不要为了凑数量重复同一个观点。
- 细节优先于漂亮措辞。
- 本段允许输出较长内容，目标约 1000-1600 个中文字符。
...
REDUCE_PROMPT = """你是一名专业的长篇访谈编辑。
下面是《The Diary Of A CEO》一集节目的“分段深度信息提取结果”。

你的任务是把这些信息整合成一份**细致、可复习、尽量接近完整节目内容地图的中文笔记**。
不要把它压缩成普通的 500 字摘要。

最重要的原则：
1. 保留具体细节、数字、案例、人物和机制。
2. 明确区分“嘉宾观点”“个人经历”“研究/事实”“推测/预测”。
3. 同一个观点如果在不同段落被展开，要合并成一个完整观点，而不是重复。
4. 如果某个观点有具体例子或数据支撑，必须把例子/数据带出来。
5. 不要为了简洁删除有信息价值的细节。
6. 不要补充文字稿之外的事实。
7. 如果某个说法需要验证，明确标注“嘉宾声称/需要进一步验证”，不要自行裁决真假。

请严格按照下面结构输出：

## 一句话总结
用 2-3 句话说清楚：
这期节目最核心的问题是什么，嘉宾给出的核心答案是什么。
...
## 节目讨论主线
用 5-10 条按逻辑顺序梳理节目从开始到结束讨论了什么。
不要简单写“聊了健康、事业、人生”，而要说明每个话题具体讨论了什么。
...
并解释每一步嘉宾是怎么论证的。
如果只是嘉宾提出的理论，不要写成已证实事实。

## 故事与案例
挑出节目中真正承担“解释观点”作用的故事。
每个故事说明：
- 发生了什么
- 涉及谁
- 它用来说明什么
...
如果无法确认原文完全准确，不要编造英文原句，可以改写为“嘉宾观点概述”。
...
### 尚需验证
哪些科学、医学、心理、商业、统计等说法在文字稿中没有充分证据。

### 预测与推测
哪些内容属于嘉宾对未来的判断。

### 争议点
哪些观点可能存在明显争议，简述争议在哪里。

## 这期节目真正值得记住的 10 件事
不要重复上面的标题。
用 10 条高信息密度的句子，把整期节目最值得长期记住的内容压缩出来。

## 适合谁看
一句话说明什么类型的人会从这期节目获得较多信息。

## 节目标签
从以下方向选择：
商业 / 创业 / AI / 科技 / 健康 / 医学 / 心理学 / 教育 /
财富 / 投资 / 人生 / 关系 / 领导力 / 运动 / 生产力 / 其他
...
def chunk_text(text, size=26000):
...
def summarize(llm, ep, text):
    ...
    for i, c in enumerate(chunks, 1):
        ...
        partials.append(llm.chat(MAP_PROMPT.format(i=i, n=len(chunks), title=ep["title"], chunk=c), max_tokens=3500))
        time.sleep(5)
    joined = "\n\n".join(f"[Part {i}]\n{p}" for i, p in enumerate(partials, 1))
    log("  reduce")
    return llm.chat(REDUCE_PROMPT.format(title=ep["title"], guest=ep["description"][:800], chunks=joined), max_tokens=7000)
